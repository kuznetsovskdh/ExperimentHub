...
cd frontend && npm run test:ui                       # 58 UI-проверок (Playwright-скрипт)
```
...
- **Аутентификации нет.** Любой, у кого есть сетевой доступ к API, может создать
  эксперимент, остановить чужой и записать события. Платформа рассчитана на
  внутренний контур; для внешнего нужен как минимум сервисный токен на
  мутирующих ручках. Осознанный скоуп: приоритет отдан корректности статистики,
  а не контуру доступа.
